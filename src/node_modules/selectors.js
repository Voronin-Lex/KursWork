...
export const getPhones1 = (state) => {
    let ret = [];
    let phonesIds = phones.map(phone => phone.id);
    let stateIds = state.admin.map(phone => phone.id);
    let i;
    for (i = 0; i < stateIds.length; i++){
        if (phones.indexOf(stateIds[i])){
            ret.push(stateIds[i]);
        }
    }
    ...
    for (let i = 0; i < ret.length; i++){
        for (let j = 0; j < phones.length; j++) {
            if (ret[i] === phones[j].id){
                retval.push(phones[j])
            }
        }
    }
